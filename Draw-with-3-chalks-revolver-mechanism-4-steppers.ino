...
float mmPerRev_Z = 69;
float PulsePermm_Z = PulsePerRev / mmPerRev_Z;    // How many pulses should be sent to motors to move 1 mm
int PulsePerRev_Revolver = 104/60*PulsePerRev;
...
#define E_STEP_PIN         26                 // Pins of motor Revolver (E in Ramps)
#define E_DIR_PIN          28
...
AccelStepper Stepper_E(1, E_STEP_PIN, E_DIR_PIN);
...
void setup() {
  ...
  Stepper_E.setMaxSpeed(Speed_Max);
  Stepper_E.setSpeed(Speed);
  Stepper_E.setAcceleration(Accl/7);
...
void processGCode(String command) {
  ...
  String z_command = command;
  z_command.remove(0, 29);                                                    // Delete first 22 letters from command line (to delete extra spaces)
  int index_Z = z_command.indexOf('Z');                                       // Position of letter "Z"
  int index_col = z_command.indexOf(';');                                     // Position of space
  float Pos_Z = z_command.substring(index_Z + 1, index_col - 1).toFloat();    // Extract Z coordinate
  
  Serial.print("Target Position:  (x ,y ,z) = ("); Serial.print(Pos_X); Serial.print(","); Serial.print(Pos_Y); Serial.print(","); Serial.print(Pos_Z); Serial.println(")");
  ...
  Serial.print("Target Position:  (sl,sr,z) = ("); Serial.print(Pos_L); Serial.print(","); Serial.print(Pos_R); Serial.print(","); Serial.print(Pos_Z); Serial.println(")");
  ...
  moveSteppers(Pos_L, Pos_R, Pos_Z, Speed_L, Speed_R, Speed_Z, Delta_L, Delta_R);      // Move the steppers regarding positions and speeds
  Serial.println("-------------------------------------");
}

void moveSteppers(float l, float r, float z, int Speed_L, int Speed_R, int Speed_Z, float Delta_L, float Delta_R) {
  if (!busy) {
    busy = true;                                // Set the flag to indicate motors are in movement

    l = l + max(Delta_L,0)*k;                   // Compensates the skipped steps of motor while loosing the rope
    r = r + max(-Delta_R,0)*k;

    Serial.print("Compensated:      (l  ,r  ) = ("); Serial.print(l); Serial.print(","); Serial.print(r); Serial.println(")");
    
    l = l * PulsePermm;                         // Convert Position (mm) to Command (Pulses)
    r = r * PulsePermm;
    z = z * PulsePermm_Z;
    ...
    while (Stepper_L.distanceToGo() != 0 || Stepper_R.distanceToGo() != 0 || Stepper_Z.distanceToGo() != 0) {    // Continue moving as long as the motors did not reach final position
      Stepper_L.run();
      Stepper_R.run();
      Stepper_Z.run();
...
void Revolve(){
  int Speed = Speed_Max/7;
  Stepper_E.setSpeed(Speed);
  Stepper_E.moveTo(PulsePerRev_Revolver);
  Serial.println("Revolver rotating...");
  while (Stepper_E.distanceToGo() != 0) {    // Continue moving as long as the motors did not reach final position
      Stepper_E.run();
    }
  Serial.println("Revolver done!");
}
